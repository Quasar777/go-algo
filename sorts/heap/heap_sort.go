...
func HeapSort(heap *MaxHeap) {
	size := len(heap.Nums)

	for i := size-1; i > 0; i-- {
		heap.Nums[0], heap.Nums[i] = heap.Nums[i], heap.Nums[0]
		heap.HeapifyDownCertainPartOfHeap(0, i)
	} 
}
...
func (h *MaxHeap) HeapifyDownCertainPartOfHeap(index int, size int) {
	largest := index
